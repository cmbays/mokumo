...
- **Boot-time install guard**: Health endpoint reports `install_ok` flag; protected routes return 423 `DEMO_SETUP_REQUIRED` when the demo installation is incomplete (admin account missing or has no password hash). Reset demo data to restore access. (#342)
