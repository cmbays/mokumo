...
- **Open Existing Shop — welcome-restore BDD test coverage**: All 20 scenarios in `welcome-restore.feature` are now wired to Playwright BDD step definitions and marked passing (removed `@wip`). Fixes 5 missing component behaviors: buttons-disabled state on "Open Existing Shop" click, direct-navigation guard on `/welcome/restore`, dismissible restore banner on the login page, "Retry" button label after restart timeout, and copy alignment ("Validating your database…", credential warning text). (#464)
