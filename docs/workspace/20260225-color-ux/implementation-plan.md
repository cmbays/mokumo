...
```typescript
// Types
type HueBucket = 'all' | 'blacks-grays' | 'whites-neutrals' | 'blues' | 'reds'
  | 'greens' | 'yellows-oranges' | 'purples-pinks' | 'browns'
...
function classifyColor(color: { family?: string; hex?: string | null; hex1?: string | null }): HueBucket
function selectRepresentativeColors(colors: Array<{ hex?: string | null; hex1?: string | null; family?: string }>, maxCount?: number): number[]  // returns indices
```
