...
**Nightly toolchain side-cars** — when a feature requires nightly Rust (e.g. branch coverage via `-Zcoverage-options=branch`, mokumo#583), the date-pinned channel lives in a sibling `rust-nightly-*-toolchain.toml` file at the workspace root, never in `rust-toolchain.toml`. A single shell script under [`scripts/`](scripts/) parses the channel and is the **only** caller from both moon tasks and CI jobs — no `nightly-YYYY-MM-DD` strings appear in any other file. The CI job consuming the side-car runs `continue-on-error: true` and is **not** in the verdict's `needs:` (matches the existing `desktop-e2e` / `crap-delta` convention) so a nightly-toolchain outage never blocks the merge queue. Promotion to a hard gate is a single-line `continue-on-error: true` removal. Removal of the whole side-car when the feature stabilizes is one PR (toml + script + job + moon task in lockstep). Existing side-car: `rust-nightly-coverage-toolchain.toml` ↔ [`scripts/nightly-coverage-channel.sh`](scripts/nightly-coverage-channel.sh) ↔ `coverage-handlers` job + `shop:coverage-branches` moon task.
