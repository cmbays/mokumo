...
// Integration tests for the restore handler live in services/api/src/restore.rs
// (the original location) because they require `build_app` which depends on the
// full services/api composition. They will move to mokumo-shop/tests/ in PR 4.

// Gated behind an impossible cfg so the test block compiles out entirely.
// These tests move to mokumo-shop/tests/ in PR 4.
#[cfg(any())]
mod tests {
