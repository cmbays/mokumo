//! Integration tests for the `mokumo-server backup` CLI subcommand.
...
fn backup_creates_timestamped_file() {
    ...
    let result = mokumo_shop::cli::cli_backup(&db_path, None).unwrap();
...
fn backup_with_custom_output_path() {
    ...
    let result = mokumo_shop::cli::cli_backup(&db_path, Some(&output_path)).unwrap();
...
fn backup_fails_for_nonexistent_db() {
    ...
    let err = mokumo_shop::cli::cli_backup(&db_path, None).unwrap_err();
    assert!(
