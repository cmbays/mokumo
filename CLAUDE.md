...
- `crates/types/` — shared types with `#[derive(FromRow, TS, Serialize)]` for both DB and TypeScript.
...
1. **Rust newtypes for entity IDs** — `struct CustomerId(i64)`, not bare `i64`. Compile-time safety at boundaries.
...
5. **Axum patterns** — see global rules at `~/.claude/rules/rust-axum.md` for server setup, SQLite PRAGMAs, error handling.
